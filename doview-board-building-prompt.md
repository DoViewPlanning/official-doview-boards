...
If the user explicitly asks for clones, cloned items, clone blocks, live copies, source-linked references, or similar on a Documentation Page, include real Documentation Page clone blocks in the relevant `savedState.docContent[pageId]` HTML. Use the existing supported marker format:
