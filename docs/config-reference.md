...
**Document status:** Technical reference for the V1.4.3 DoView Boards prompt package release

This document describes the board configuration structure used by the V1.4.3 DoView Board reference engine and builder.
...
node doview-board-builder.js \
  ...
  --out example-board_doview-board_v1.4.3_2026-08-14.html
```
...
For a single box, include each Measure ID or Evaluation Question ID at most once. The V1.4.3 reference engine de-duplicates repeated box-level Measure/Evaluation Question IDs when loading or saving a board, without changing the saved-state field names or Measure/Evaluation Question definitions.
...
How links do not support link-level Measures or Evaluation Questions in V1.4.3. Do not add `measures` or `evalQuestions` to How links expecting the reference engine to use them.
...
Standard V1.4.3 setup settings:
...
The V1.4.3 builder scans visible generated board content for `http://` and `https://` URLs, deduplicates URL-bearing source entries, and safely adds missing content/evidence registry entries using the URL as the title when no better title is available. It excludes fixed package-controlled help, training, repository, trademark, and support URLs from auto-addition. It does not invent URLs, and it preserves explicitly titled source entries so genuine board-content sources are not removed.
...
```text
example-board_doview-board_v1.4.3_2026-08-14.html
```
...
- do not add unsupported schema and assume the V1.4.3 reference engine will use it.
