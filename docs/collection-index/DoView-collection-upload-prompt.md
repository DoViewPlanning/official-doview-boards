...
2. If the overall collections page itself is being created or changed, the current `/boards/index.html` file from GitHub. Use `templates/00-collection-of-collections-index-template.html` if it is missing or if the user wants to rebuild it from the standard collection-of-collections template.
...
This package also includes `templates/00-collection-of-collections-index-template.html`. Use it only for the top-level collection-of-collections page that lives at:
...
Use this when every box is a live DoView Board. There is no centered top board and no placeholder boxes. Example: an International Development collection containing five live boards.

**Option 2 — Top Board Plus Live Boards Collection**

Use this when there is one centered Top DoView Board, followed by a collection of live DoView Boards. Do not include the yellow/orange instruction strip unless there are placeholders.

**Option 3 — Live boards plus placeholders**

Use this when some boxes are live DoView Boards and other boxes are illustrative placeholders. This option may be used with or without a centered Top DoView Board. Only this option should include the yellow/orange instruction strip:
...
- Public reusable collection templates are badge-free by default.
- Do not add a collection-level Official DoView Badge to a public reusable template. Collection-level official badges are reserved for official curated DoView collections.
- Keep the fixed top-right acknowledgment block unchanged unless the user has a specific approved reason to alter it. The acknowledgment heading should be `A DoView Board Collection`, followed by separate lines for `Acknowledgment:`, `DoView Boards`, `by Dr Paul Duignan`, and linked `DoViewPlanning.Org`.
- Individual board HTML files may still carry their own board-level Official DoView Badge if they were generated by the official prompt/package.
- Users should normally edit only title, subtitle, maintainer/publisher line, description/about text, board entries, collection entries, counts, and links.
- Preserve the card visual design and JSON-loading behaviour.
...
```json
{
  ...
  "maintainer": "Collection maintained by: [your name / organization]",
  "currentTitle": "DoView Board Collection",
  "status": "Illustrative only - Not Endorsed",
  "sectionTitle": "DoView Boards",
  "featured": null,
  "boards": [
    {
      "title": "Example Board",
...
- `5 boards`
- `1 top board · 5 live boards · 17 placeholders`
